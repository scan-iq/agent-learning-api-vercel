 ...
 */
class VercelKVCache implements CacheBackend {
  private kv: any;
 ...
 */
class TwoTierCache implements CacheBackend {
  private l1: MemoryCache;
  private l2: CacheBackend;

  constructor(l1: MemoryCache, l2: CacheBackend) {
    this.l1 = l1;
    this.l2 = l2;
  }
...
export class Cache {
  ...
  constructor(backend: CacheBackend, name: string = 'default') {
...
let rateLimitCache: Cache | null = null;
let queryCache: Cache | null = null;

/**
 * Initialize caches
 */
export function initializeCaches(kv?: any): void {
 ...
 * Get auth cache
 */
export function getAuthCache(): Cache {
  if (!authCache) {
    initializeCaches();
  }
  return authCache!;
 ...
 */
export function getRateLimitCache(): Cache {
  if (!rateLimitCache) {
    initializeCaches();
  }
 ...
 * Get query cache
 */
export function getQueryCache(): Cache {
  if (!queryCache) {
    initializeCaches();
  }
  return queryCache!;
...
export function getCacheStats(): {
  auth: { hitRate: number };
  rateLimit: { hitRate: number };
  query: { hitRate: number };
} {
  return {
...
export const httpCache = {
  ...
  setCacheHeaders(
    ...
    options: {
      ...
      sMaxAge?: number; // seconds (CDN/edge)
 ...
 */
export function setVaryHeader(res: any, headers: string[]): void {
