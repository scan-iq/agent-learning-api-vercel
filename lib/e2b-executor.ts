...
export class IrisCodeExecutor {
   ...
   */
  private async setupIrisEnvironment(
    sandbox: Sandbox,
    options: {
      projectId?: string;
      sessionId?: string;
...
`;
   ...
   */
  private formatOutput(execution: any): string {
    const parts: string[] = [];
    ...
    if (execution.logs?.stdout && execution.logs.stdout.length > 0) {
      parts.push('\n=== Output ===');
      execution.logs.stdout.forEach((msg: any) => {
    ...
    if (execution.logs?.stderr && execution.logs.stderr.length > 0) {
      parts.push('\n=== Errors ===');
      execution.logs.stderr.forEach((msg: any) => {
        parts.push(msg.line || msg.toString());
      });
    }
  ...
  private formatLogs(logs: any): string[] {
    ...
    if (logs?.stdout) {
      logs.stdout.forEach((msg: any) => {
        formatted.push(`[stdout] ${msg.line || msg.toString()}`);
      });
    }

    if (logs?.stderr) {
      logs.stderr.forEach((msg: any) => {
  ...
  async closeAll(): Promise<void> {
    const cleanupPromises = Array.from(this.activeSandboxes.values()).map(
      sandbox => this.cleanupSandbox(sandbox)
    );
 ...
 */
let executorInstance: IrisCodeExecutor | null = null;

export function getExecutor(config?: E2BExecutorConfig): IrisCodeExecutor {
  if (!executorInstance) {
