 ...
 */
export function checkRateLimit(key, maxRequests = 100, windowMs = 60_000) {
    startCleanupTimer();
    const now = Date.now();
 ...
 * @param windowMs - Time window in milliseconds
 */
export function rateLimit(key, maxRequests = 100, windowMs = 60_000) {
 ...
 */
export function getRateLimitStatus(key, maxRequests = 100) {
    const entry = rateLimitStore.get(key);
 ...
 */
export function rateLimitByIp(ip, maxRequests = 100, windowMs = 60_000) {
    rateLimit(`ip:${ip}`, maxRequests, windowMs);
}
/**
 * Rate limit by API key / project ID
 */
export function rateLimitByApiKey(projectId, maxRequests = 1000, windowMs = 60_000) {
    rateLimit(`apikey:${projectId}`, maxRequests, windowMs);
}
/**
 * Combined rate limiting - checks both IP and API key
 */
export function rateLimitCombined(ip, projectId, ipLimits = { maxRequests: 100, windowMs: 60_000 }, apiKeyLimits = { maxRequests: 1000, windowMs: 60_000 }) {
