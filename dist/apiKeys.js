...
export async function createApiKey(params) {
    ...
    if (error) {
        if (error.code === "23505") {
            ...
            throw new Error(`API key with label "${params.label}" already exists for this project`);
        }
 ...
 */
export async function findProjectByApiKey(apiKey) {
    const supabase = getSupabaseClient();
    const apiKeyHash = hashApiKey(apiKey);
    const { data, error } = await supabase
        .from("iris_api_keys")
 ...
 */
export async function touchApiKeyUsage(keyId) {
    const supabase = getSupabaseClient();
